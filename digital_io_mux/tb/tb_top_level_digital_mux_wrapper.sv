...
module tb_top_level_digital_mux_wrapper();
...
// signals for apb_interface
apb_if apbif(.PCLK(tb_clk), .PRESETn(tb_n_rst));
...
assign apbif.PADDR = tb_paddr;
assign tb_prdata = apbif.PRDATA;
assign apbif.PWDATA = tb_pwdata;
assign apbif.PSEL = tb_psel;
assign apbif.PENABLE = tb_penable;
assign apbif.PWRITE = tb_pwrite;
assign apbif.PSTRB = tb_pstrb;
assign tb_pready = apbif.PREADY;
assign tb_pslverr = apbif.PSLVERR;
...
// DUT
//*****************************************************************************
top_level_digital_mux_wrapper DUT
(
