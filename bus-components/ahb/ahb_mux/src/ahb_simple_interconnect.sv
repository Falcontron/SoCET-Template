...
module ahb_simple_interconnect #(
    ...
    function logic addr_match(logic [31:0] bus_addr, int subordinate_idx);
        return (bus_addr >= AHB_MAP[subordinate_idx] && (subordinate_idx == NSUBORDINATES-1 || bus_addr < AHB_MAP[(subordinate_idx + 1) % NSUBORDINATES]));
    endfunction
    ...
    generate
        for(i = 0; i < NSUBORDINATES; i++) begin : g_request
            ...
            assign subordinates[i].HSEL         = (manager.HTRANS != ahb_pkg::IDLE && addr_match_arr[i] && manager.HREADYOUT);
        end
    endgenerate
    ...
    generate
        for(i = 0; i < NSUBORDINATES; i++) begin : g_pack_response
