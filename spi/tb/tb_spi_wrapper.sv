module tb_spi_wrapper();
    ...
    /*assign busif.addr = PADDR;
    assign busif.wdata = PWDATA;
    assign PRDATA = busif.rdata;
    assign busif.wen = PSEL & PENABLE & PWRITE;
    assign busif.ren = PSEL & PENABLE & !PWRITE;*/
    assign busif.strobe = 4'b1111;
    ...
    task apb_txn(
        ...
        // Data Phase
        #(1);
        return_value = busif.rdata;
        // Check PRDATA if needed
        if(!write && check) begin
            assert(return_value == expected)
            else $display("In block %s, test %d:\n\tIncorrect Transaction, expected %h, received %h", test_block, test_num, expected, return_value);
        end
        @(posedge CLK);
        #(1);
        busif.wen = 0;
