...
`endif
    ...
    ************************/
    ...
    plic_if #(.NUM_INTERRUPTS(NUM_INTERRUPTS), .NUM_CONTEXTS(NUM_HARTS)) plicif(); //Originally 8 interrupts
    clint_if #(.NUM_HARTS(NUM_HARTS)) clif();

    /*****************************
    * Bus Interconnect Interfaces
    ******************************/
    ahb_if managers[2](HCLK, ahb_nRST);
    ahb_if ahb_peripherals[AHB_NSUBORDINATES](
        .HCLK(HCLK),
        .HRESETn(ahb_nRST)
    );
    ahb_if muxed_ahb_if(HCLK, ahb_nRST);

    apb_if apb_requester_if(PCLK, apb_nRST);
    apb_if apb_peripherals[APB_NCOMPLETERS](PCLK, apb_nRST);
    ...
    ************************/
    ...
    // AHB Peripherals
    ahb2apb #(
        .NWORDS(APB_NCOMPLETERS * 4096),
        .FIFO_DEPTH(1),
        .BASE_ADDR(AHB_MAP[APB_BRIDGE_IDX])
    ) BRIDGE(
        .ahbif(ahb_peripherals[APB_BRIDGE_IDX]),
...
`ifdef LOADABLE_FW // if loadable firmware is enabled, loadable rom    
    ff_ram #(
        .NBYTES(ROM_SIZE),
        .ROM(1)
    ) ROMSIM (
...
`endif
    ...
    ******************/
    ...
    plic #(.NUM_INTERRUPTS(NUM_INTERRUPTS), .NUM_CONTEXTS(NUM_HARTS)) PLIC(
        .CLK(HCLK),
        .nRST(ahb_nRST),
        .busif(plic_protif),
        .plicif
...
`ifdef LOADABLE_FW
    `ADD_AHB(ff_ram_rom, ROM_AHB_IDX, ROM_N_WORDS, rom_protif)
`else
...
`endif
    ...
    generate
        for(j = 0; j < NUM_GPIO_PORTS; j++) begin : g_gpio_port_irq
            for(i = 0; i < GPIO_PINS_PER_PORT; i++) begin : g_gpio_irq
                `DEFINE_INTERRUPT(gpio, (j*GPIO_PINS_PER_PORT + i), gpioifs[j].irq[i])
            end
        end
    endgenerate
    ...
    generate
        for (i = GPIO_PINS_PER_PORT*NUM_GPIO_PORTS; i < GPIO_PINS_PER_PORT*NUM_GPIO_PORTS + 6; i++) begin : g_spi_irq
            `DEFINE_INTERRUPT(spi, i, spiif.interrupts[i-GPIO_PINS_PER_PORT*NUM_GPIO_PORTS])
        end
    endgenerate

    // DMA
    `DEFINE_INTERRUPT(dma, GPIO_PINS_PER_PORT*NUM_GPIO_PORTS + 6, dma_interrupt)

    // TIMER
    generate
        for(i = GPIO_PINS_PER_PORT*NUM_GPIO_PORTS + 7; i < TIMER_CC_CHANNELS + GPIO_PINS_PER_PORT*NUM_GPIO_PORTS + 7; i++) begin : g_timer_irq
            `DEFINE_INTERRUPT(timer, i, timerif.t_irq[i - (GPIO_PINS_PER_PORT*NUM_GPIO_PORTS + 7)])
        end
    endgenerate
    `DEFINE_INTERRUPT(timer_tc, GPIO_PINS_PER_PORT*NUM_GPIO_PORTS + TIMER_CC_CHANNELS + 7, timerif.tc_irq)

    generate
        for (i = TIMER_CC_CHANNELS + GPIO_PINS_PER_PORT*NUM_GPIO_PORTS + 8; i < 64; ++i) begin
    ...
    // CLINT
    generate
        for(i = 0; i < NUM_HARTS; i++) begin : g_irqif
            assign irqif.soft_int[i] = clif.soft_int[i];
            assign irqif.soft_int_clear[i] = clif.clear_soft_int[i];
            assign irqif.timer_int[i] = clif.timer_int[i];
            assign irqif.timer_int_clear[i] = clif.clear_timer_int[i];
        end
    endgenerate

    // PLIC
    generate
      for(i = 0; i < NUM_HARTS; i++) begin : g_irq_plic 
          assign irqif.ext_int[i] = plicif.interrupt_service_request[i];
          assign irqif.ext_int_clear[i] = ~plicif.interrupt_service_request[i];
      end
    endgenerate 
